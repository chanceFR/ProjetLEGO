...
import fr.antoromeochrist.projetlego.openjfxapi.PolygonMesh;
import fr.antoromeochrist.projetlego.openjfxapi.PolygonMeshView;
...
public class GridUtils {
    ...
    private Group createGrid(float size, float delta) {
        if (delta < 1) {
            delta = 1;
        }
    ...
    public static Group getAxes(double scale) {
        ...
        axisX.setMaterial(new PhongMaterial(Color.web("#A52A2A")));

        Cylinder axisY = new Cylinder(1, 200);
        axisY.getTransforms().add(new Translate(0, 100, 0));
        axisY.setMaterial(new PhongMaterial(Color.web("#7FFF00")));

        Cylinder axisZ = new Cylinder(1, 200);
        axisZ.setMaterial(new PhongMaterial(Color.web("#8A2BE2")));
